...
public class StudentService {
   ...
   private static final int BUFFER_SIZE = 4096;
	...
	public boolean addStudent() {
			...
			List<FileItem> items = new ServletFileUpload(new DiskFileItemFactory()).parseRequest(request);
			 ...
			 for (FileItem item : items) {
		            if (item.isFormField()) {
						...
						/*
						 * if (fieldName.equalsIgnoreCase("collegename")) { String[] colg =
						 * (DataUtil.emptyString(item.getString())).split("_"); collegeName = colg[1];
		...
		student.setStudentexternalid("AA"+DataUtil.generateString(4));
		...
		*/
		...
		if(parents.getStudent().getSid()!=null){
			result=true;
		}
